...
export function createLimiter({ minIntervalMs = 500, maxPerHour = 500 } = {}) {
  ...
  // limiting, so it falls back to the default 500 rather than Infinity. To opt out
  // of the hourly cap entirely, pass Infinity explicitly.
  const cap = maxPerHour === Infinity ? Infinity : (Number.isFinite(maxPerHour) && maxPerHour >= 1 ? Math.floor(maxPerHour) : 500);

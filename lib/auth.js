  ...
  async function request(path, { method = 'GET', body, signal, timeoutMs = 20000 } = {}) {
    ...
    if (signal) {
      if (signal.aborted) {
        ...
        // Fallback for engines without AbortSignal.any: mirror the caller abort,
        // and REMOVE the listener when the request settles (see cleanup) so a
        // reused caller signal doesn't accumulate a listener per request.
        onCallerAbort = () => { try { ac.abort(); } catch (_e) {} };
        try { signal.addEventListener('abort', onCallerAbort); } catch (_e) { onCallerAbort = null; }
      }
    }
    const cleanup = () => {
      clearTimeout(to);
      if (onCallerAbort) { try { signal.removeEventListener('abort', onCallerAbort); } catch (_e) {} onCallerAbort = null; }
    };

...
(function () {
  ...
  function buildHar() {
    return {
      log: {
        version: '1.2',
        creator: { name: 'GrindrBlock userscript', version: '0.43.0' },
  ...
  function greetViaUi(profileId) {
    ...
    const goHome = () => {
      ...
      if (!GREET_RETURN_AFTER_SEND) return;
      setTimeout(() => {
        // Escape closes Grindr's profile/chat overlays.
        pressEscape();
        setTimeout(() => {
          // Route to the GRID, never back to the captured `from`. When the greet
          // started on an open profile, `from` is "/?profile=true" — a flag with
          // no profile state behind it — and pushing it back produced the blank
          // screen reported after a successful send. '/' always renders.
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.43.0' });

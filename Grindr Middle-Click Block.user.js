...
(function () {
  ...
  function buildHar() {
    return {
      log: {
        version: '1.2',
        creator: { name: 'GrindrBlock userscript', version: '0.46.0' },
  ...
  function upgradeHidesToBlocks(limit) {
    ...
    logInfo(`${LOG} queued ${todo.length} hide→block upgrade(s); ${hidesNeedingUpgrade().length - todo.length} would remain.`);
    showToast(`Upgrading ${todo.length} hide(s) to real blocks…`, 'ok');
    refreshHud();
    return todo.length;
  }
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.46.0' });

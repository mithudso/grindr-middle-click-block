...
(function () {
  ...
  async function drainTick() {
    ...
    try {
      ...
      try { await reconcileBlockTiers(false, true); } catch (_e) {}   // the drain's own bookkeeping, on the drain's budget
  ...
  async function processQueue() {
    ...
    try {
      while (blockQueue.length) {
        ...
        pruneCallWindows();
        const manualCapped = recentManualCalls.length >= MANUAL_HOURLY_CAP;
        const bulkCapped = recentBulkCalls.length >= DRAIN_HOURLY_CAP;
  ...
  try {
    if (typeof module === 'object' && module && module.exports) {
      module.exports = {
        ...
        queueInsertIndex, nextRunnableIndex,

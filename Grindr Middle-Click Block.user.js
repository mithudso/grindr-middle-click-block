...
(function () {
  ...
  // True once the drain has used its share of the rolling-hour cap, at which point
  // it holds so the remainder stays available for blocks you press yourself.
  function drainBudgetSpent() {
    const hourAgo = Date.now() - 3_600_000;
    const used = recentBlockTimestamps.filter((t) => t >= hourAgo).length;
    return used >= Math.floor(MAX_PER_HOUR * DRAIN_BUDGET_FRACTION);
  }
  ...
  function enqueueAction(profileId, action, opts) {
    ...
    if (bulk) blockQueue.push(job);
    else {
      // Interactive: ahead of every bulk job, behind other interactive ones.
      const firstBulk = blockQueue.findIndex((j) => j.bulk);
      if (firstBulk < 0) blockQueue.push(job); else blockQueue.splice(firstBulk, 0, job);
    }

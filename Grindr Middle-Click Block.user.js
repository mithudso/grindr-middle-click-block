...
(function () {
  ...
  const HOTKEY_GREET_KEY = 'Insert';
  const HOTKEY_ALBUM_KEY = 'Delete';
  const HOTKEY_BLOCK_KEY = 'Home';
  const HOTKEY_HIDE_KEY = 'End';
  const HOTKEY_PREV_KEY = 'PageUp';
  const HOTKEY_NEXT_KEY = 'PageDown';
  ...
  function saveHiddenList() {
    try {
      localStorage.setItem(HIDELIST_STORAGE_KEY, JSON.stringify(Object.fromEntries(hiddenProfileIds)));
      return true;
    } catch (e) { logWarn(`${LOG} saveHiddenList failed:`, e); return false; }
  }
  ...
  function enforceAllBlocked() {
    ...
    // …and the persistent PageDown hide list, which uses the identical DOM-only
    // collapse. Same enforcer, different source of ids and a different lifetime.
    for (const pid of hiddenProfileIds.keys()) {
      try { enforceTextHiddenProfile(pid); } catch (e) { logTrace(`${LOG} enforceHiddenProfile error:`, e); }
    }
  }
  ...
  function findSendButton(inputEl) {
    // No document fallback, for the reason findProfileChatButton has none: a
    // page-wide search puts nav/header controls accessibly named "Send" into the
    // candidate pool, and this matcher is what types into a chat box.
    const scope = inputEl && inputEl.closest("form, [class*='chat'], [class*='message'], [class*='conversation']");
    if (!scope) { logTrace(`${LOG} findSendButton: composer has no chat-scope ancestor — refusing a page-wide search`); return null; }
    let buttons = [];
  ...
  const OPEN_CONVERSATION_MAX_AGE_MS = 60_000;
  ...
  function dismissAccidentalPanel() {
  ...
  function greetViaUi(profileId) {
    ...
    const goHome = () => {
      ...
      if (!GREET_RETURN_AFTER_SEND) return;
      setTimeout(() => {
        // Escape closes Grindr's profile/chat overlays; the route push is the
        // backstop. '/' is the grid and needs no conversation id, so this is safe
        // in a way that /chat/<id> was not.
        try { document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', keyCode: 27, which: 27, bubbles: true })); } catch (_e) {}
        setTimeout(() => {
          if (location.pathname + location.search !== from) spaNavigate(from || '/');
        }, 250);
      }, GREET_RETURN_DELAY_MS);
    };
    ...
    const pressChatButton = () => closeLightbox(() => pollFor(
      ...
      () => { logWarn(`${LOG} greet: no Chat button on the open profile for ${id}`); showToast('Greet failed: no Chat button', 'warn'); goHome(); }
    ));

    // Profile already open → straight to its Chat button.
    if (isProfileViewOpen()) { pressChatButton(); return true; }
  ...
  function hotkeyGreetTarget() {
    ...
    // Say so, and DON'T advance — the next Insert should retry this profile.
  ...
  function hotkeyHideTarget() {
    ...
    if (blockedProfileIds.has(id)) {
      // Already blocked for real; a DOM-only hide on top would be a no-op that
      // looks like it did something. Say so instead.
  ...
  let albumState = { discovered: [], shares: {}, order: [], names: {}, myProfileId: '', myProfileIdSource: '', listUrl: '', stateVersion: ALBUM_STATE_VERSION };
  ...
  document.addEventListener('keydown', (e) => {
    ...
    const eq = (k) => key.toLowerCase() === String(k).toLowerCase();
    const isGreet = eq(HOTKEY_GREET_KEY);
    const isAlbum = eq(HOTKEY_ALBUM_KEY);
    const isBlock = eq(HOTKEY_BLOCK_KEY);
    const isHide = eq(HOTKEY_HIDE_KEY);
  ...
  window.__grindrBlock_hotkeys = function () {
    return {
      ...
      keys: {
        next: HOTKEY_NEXT_KEY, prev: HOTKEY_PREV_KEY,
        greet: HOTKEY_GREET_KEY, album: HOTKEY_ALBUM_KEY,
        block: HOTKEY_BLOCK_KEY, hide: HOTKEY_HIDE_KEY,
      },
  ...
  logInfo(`${LOG} loaded v0.30.0 (LOCAL_ONLY=${LOCAL_ONLY}, MIN_INTERVAL_MS=${MIN_INTERVAL_MS}, MAX_PER_HOUR=${MAX_PER_HOUR}, UNDO_WINDOW_MS=${UNDO_WINDOW_MS}, VERIFY_BLOCKS=${VERIFY_BLOCKS}, blockList=${blockedProfileIds.size}, hideList=${hiddenProfileIds.size}, textFilter=${TEXT_FILTER_KEYWORDS.length ? TEXT_FILTER_ACTION : 'off'}, stayLoggedIn=${STAY_LOGGED_IN}, skipBetaDialog=${SKIP_BETA_DIALOG}, hotkeys=${HOTKEYS_ENABLED ? `${HOTKEY_GREET_KEY} greet, ${HOTKEY_ALBUM_KEY} album, ${HOTKEY_BLOCK_KEY} block, ${HOTKEY_HIDE_KEY} hide, ${HOTKEY_PREV_KEY}/${HOTKEY_NEXT_KEY} nav` : 'off'}, albums=${albumRotation().length}, greetMode=${GREET_MODE}, me=${albumState.myProfileId || 'unknown'}${myProfileIdIsSeeded ? ' (seeded)' : ''})`);
})();

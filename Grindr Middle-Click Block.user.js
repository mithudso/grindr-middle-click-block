...
(function () {
  ...
  function diagReset() { diagTrace = []; diagSignal = []; diagNoisyCount = 0; diagNoisySkipped = 0; }
  ...
  function isSixKey(k) {
    ...
    return [HOTKEY_GREET_KEY, HOTKEY_ALBUM_KEY, HOTKEY_BLOCK_KEY, HOTKEY_HIDE_KEY, HOTKEY_PREV_KEY, HOTKEY_NEXT_KEY]
      .some((x) => String(x).toLowerCase() === key);
  }
  ...
  function renderHud() {
    ...
    diag.appendChild(mkBtn('reconcile', (ev) => {
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.35.0' });
  ...
  function saveDiagReport() {
    ...
    try {
      ...
      showToast(`Saved ${diagCount()} entries`, 'ok');
    } catch (e) {

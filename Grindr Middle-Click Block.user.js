...
(function () {
  ...
  // The drain converts local entries from hides into /api/v3/me/blocks writes.
  // With BLOCK_MODE back to 'hide' that is converting the action that works into
  // one that does not, so it no longer starts on its own. The button still exists
  // for anyone who wants to run it deliberately.
  const AUTO_DRAIN_DEFAULT = false;
  ...
  function installAutoDrain() {
    ...
    // A drain switched on under an older version was switched on for a conversion
    // we now believe is counterproductive, so do not silently resume it.
    autoDrain = o ? (!!o.on && AUTO_DRAIN_DEFAULT) : AUTO_DRAIN_DEFAULT;
  ...
  // 'hide'  — POST /api/v1/me/hides/{id}, which is what GRINDR'S OWN menu fires.
  // 'block'  — POST /api/v3/me/blocks/{id}, the endpoint v0.45 switched to.
  //
  // Back to 'hide' in v0.62.0 on direct evidence. A capture of the operator using
  // Grindr's own UI shows their action is, verbatim:
  //
  //     POST https://web.grindr.com/api/v1/me/hides/{id}
  //
  // and it sticks — the profile goes. Ours, in the same capture, is
  // POST /api/v3/me/blocks/{id} answered 200 {"updateTime":0}, and the tile stays.
  // The operator's summary was "no blocking mechanism sticks except for theirs".
  //
  // That inverts v0.45's premise. /api/v3/me/blocks does populate a blocks list
  // (a paginated walk reads 1656 of them, so the writes are real in some sense)
  // but it is evidently not the relationship that removes someone from the feed.
  // Copying the app exactly is the whole reason this script's other endpoints work.
  //
  // The consequence is uncomfortable and worth stating plainly: the hide→block
  // drain has been converting the action that WORKS into the one that does not.
  // It is therefore off by default from v0.62.0 (see AUTO_DRAIN_DEFAULT).
  const BLOCK_MODE = 'hide';
  ...
  async function runUnblockJob(job, auth, jobKey) {
    const { profileId, action } = job;
        ...
        if (action === 'unblock') {
          ...
          // Reverse whatever that job would have written. Bulk (drain) work uses
          // the blocks endpoint; a gesture uses whatever BLOCK_MODE says, which is
          // the hide — the action Grindr's own UI performs.
          const undoMode = job.bulk ? 'block' : BLOCK_MODE;
          const result = await (undoMode === 'block'
  ...
  async function runBlockJob(job, auth, jobKey) {
    const { profileId, action } = job;
        ...
        //   * a GESTURE writes what Grindr's own menu writes — POST
        //     /api/v1/me/hides/{id} — because that is the one observed to actually
        //     remove someone from the feed;
        //   * the DRAIN writes POST /api/v3/me/blocks/{id}, since converting local
        //     entries into real blocks-list entries is the only thing it is for.
        // Conflating them is what made a middle-click send a write that returned
        // 200 and changed nothing.
        const writeMode = job.bulk ? 'block' : BLOCK_MODE;
        const result = await (writeMode === 'block'
  ...
  function enqueueAction(profileId, action, opts) {
    ...
    // Both skips below are about the /api/v3/me/blocks endpoint ONLY, so they may
    // apply only to a job that will actually write there — i.e. bulk drain work.
    //
    // They were added in v0.61.0 while every gesture wrote /blocks. v0.62.0 moved
    // gestures to the hide, and leaving these unguarded would have been worse than
    // the bug they fixed: a profile in the blocks list (1656 of them) would have
    // had its hide skipped, and the hide is the write that actually removes
    // someone. "Already in the blocks list" says nothing about whether a profile
    // is hidden — the two are different relationships, which is the entire lesson
    // of v0.62.0.
    if (action === 'block' && !LOCAL_ONLY && bulk) {
      if (serverBlockedIds.has(profileId)) {
        logInfo(`${LOG} ${profileId} is already in Grindr's blocks list — not re-POSTing a block.`);
        return;
      }
      if (isUnupgradeable(profileId)) {
        logInfo(`${LOG} ${profileId} will not accept a block (200 but never listed) — skipping the write.`);
        return;
      }
    }
  ...
  function resolveProfileIdFromClick(e) {
    ...
    // Strategy 0: the context names the profile without any card.
    //   • A click INSIDE the open profile overlay is a click on the profile it
    //     shows — asked of the overlay's own React fiber (openProfileViewId). The
    //     old walk-up from here found a container holding the profile's photo and
    //     then read the first photo hash inside it, which for the overlay root was
    //     a CASCADE TILE behind the modal: the top-left tile, every time.
    //   • On the bare /chat route, the conversation Grindr fetched names the peer.
    //     A /chat/<a>:<b> id is SORTED, so half[0] is the signed-in user roughly
    //     half the time — this used to block or greet YOURSELF on a coin flip;
    //     chatPeerIdFromPath resolves the peer correctly for both shapes. It is
    //     consulted ONLY on /chat: an inbox refresh also fetches a conversation,
    //     and off the chat page that named whoever had just messaged you.
    const overlay = findProfileOverlayRoot();
    if (overlay && overlay.contains(target)) {
      const shown = openProfileViewId();
      return shown ? { profileId: shown, profileEl: null, source: 'overlay-fiber' } : { profileId: '', profileEl: null };
    }
    if (isOnChatPage()) {
  ...
  function attemptBlock(e) {
    ...
    const { profileId, profileEl, source } = resolveProfileIdFromClick(e);
    if (!profileId) {
      const t = e.target;
    ...
    // A gesture on the OPEN profile gets the same after-action as the Home key,
    // so middle-click and Home behave alike (the rule that came from the card
    // collapse living in one wrapper). Without it the overlay is left pointing at
    // the card the hide just removed: a blank pane whose only photos are the
    // pager's neighbour thumbnails.
    if (source === 'overlay-fiber') applyAfterAction(settings.afterBlock, profileId, false);
    return true;
  }
  ...
  function chatPeerIdFromPath() {
    try {
      ...
      // Only on /chat itself (v0.65.0). An inbox refresh fetches a conversation
      // too, and off the chat page that named whoever had just messaged you.
      if (isOnChatPage() && openConversation.id && Date.now() - openConversation.at <= OPEN_CONVERSATION_MAX_AGE_MS) {
  ...
  function contradictedByOpenProfile(id) {
    ...
    // The overlay's own fiber says who is on screen. The observed conversation
    // used to stand in for it, and an inbox refresh can point that at whoever
    // just messaged you — so it is consulted only where it IS the screen: /chat.
    const shown = openProfileViewId() || (isOnChatPage() ? openProfilePeerId() : '');
    if (!shown || !isPlausibleProfileId(id)) return false;
  ...
  // There is no reason to click Chat every time; opening a profile almost always
  // means you are about to talk to them. This clicks the profile's own Chat
  // button once per opened profile.
  //
  // Deliberately narrow. It uses findProfileChatButton, which is scoped to the
  // open profile view and returns null rather than guessing — the same resolver
  // the greet flow uses, and the reason a greeting stopped going to the wrong
  // person. If the button is not there (Grindr removes it for a profile you have
  // blocked) nothing happens, and we do not search the page for a substitute.
  const AUTO_CHAT_POLL_MS = 400;
  let autoChatLastProfile = '';
  let autoChatTimer = 0;
  // One check: if a different profile is open than the last one acted on, and its
  // Chat button is present, click it.
  function autoOpenChatTick() {
    if (SCRIPT_DISABLED || !settings.autoOpenChat) return;
    let open = false;
    try { open = isProfileViewOpen(); } catch (_e) { return; }
    if (!open) { autoChatLastProfile = ''; return; }   // reset, so re-opening acts again
    let pid = '';
    // The overlay's fiber, not the observed conversation: a fresh profile has no
    // conversation yet (that is what clicking Chat creates), so the old source
    // returned '' here and this tick could never act on anyone new.
    try { pid = openProfileViewId() || ''; } catch (_e) {}
    if (!pid || pid === autoChatLastProfile) return;
    let btn = null;
    try { btn = findProfileChatButton(); } catch (_e) { return; }
    if (!btn) return;                                  // not rendered yet, or a blocked profile
    autoChatLastProfile = pid;                         // claim it BEFORE clicking, so a
                                                       // re-render cannot double-fire
    try {
      btn.click();
      logInfo(`${LOG} auto-opened the chat card for ${pid}.`);
      diagEvent('auto-open-chat', { profileId: pid });
    } catch (e) { logTrace(`${LOG} auto-open chat failed:`, e); }
  }
  // Start the poll. A poll rather than a MutationObserver because the button
  // appears some frames after the route change, and an observer fires long before
  // it exists.
  function installAutoOpenChat() {
    autoChatTimer = setInterval(() => {
      try { autoOpenChatTick(); } catch (e) { logTrace(`${LOG} autoOpenChatTick error:`, e); }
    }, AUTO_CHAT_POLL_MS);
    installedIntervals.push(autoChatTimer);
  }
  ...
  // On the current build the overlay is a MuiModal appended to <body> that
  // carries the profile pager's arrows. Nothing else on the page is labelled
  // "Next Profile"/"Previous Profile", so the arrows identify the overlay the way
  // "close drawer" identifies the chat drawer. Observed live 2026-08-31:
  //   div.MuiModal-root[role="presentation"] > … [aria-label="Previous Profile"],
  //   [aria-label="Next Profile"], button[aria-label="Close"], the "Say
  //   something..." composer input, and the profile's photos (largest ≈1361px).
  const PROFILE_OVERLAY_MARKER = '[aria-label="Next Profile" i], [aria-label="Previous Profile" i]';
  // The overlay's root element, or null. Only when the URL says one is open.
  function findProfileOverlayRoot() {
    if (!isProfileOverlayOpenFromUrl()) return null;
    let best = null;
    let bestArea = 0;
    try {
      for (const m of document.querySelectorAll('.MuiModal-root, [role="presentation"], [role="dialog"]')) {
        if (isOwnGreetUi(m) || !isVisibleEl(m)) continue;
        if (!m.querySelector(PROFILE_OVERLAY_MARKER)) continue;
  ...
  // The profile id carried by a component above `el`, restricted to the prop
  // shapes the overlay actually uses. Observed from the composer input, the
  // heading and the main photo: `userProfile.profileId` at depths 5–13 and
  // `profile.profileId` at 18–25, all naming the same id, which changed on every
  // pager step while the rest of the page did not. Deliberately NOT the generic
  // findProfileIdInFiber: that one accepts any `id` prop it meets, and above the
  // overlay there are components whose `id` is not a profile.
  const VIEW_FIBER_MAX_DEPTH = 40;
  // A photo anchor must be pane-sized: the main photo measures ≈1361px, the
  // pager's Previous/Next thumbnails 40px.
  const VIEW_PHOTO_MIN_PX = 200;
  // How many anchors must answer (and agree) before the id is believed.
  const VIEW_MIN_ANCHORS = 2;
  // The profile id a component above `el` carries, or '' (see the note above).
  function profileIdFromViewFiber(el) {
    if (!el) return '';
    let keys = [];
    try { keys = Object.keys(el).filter((k) => k.startsWith('__reactFiber$') || k.startsWith('__reactInternalInstance$')); } catch (_e) { return ''; }
    if (!keys.length) return '';
    let fiber = el[keys[0]];
    for (let depth = 0; fiber && depth < VIEW_FIBER_MAX_DEPTH; depth += 1, fiber = fiber.return) {
      const p = fiber.memoizedProps;
      if (!p || typeof p !== 'object') continue;
      for (const v of [p.userProfile && p.userProfile.profileId, p.profile && p.profile.profileId, p.profileId]) {
  ...
  // Exactly one distinct plausible id → that id; none, or a disagreement → ''.
  // A disagreement means at least one anchor sits in a component for a DIFFERENT
  // person, and acting on either half is acting on a guess.
  function pickAgreedId(ids) {
    const set = new Set();
    for (const id of ids || []) { const s = String(id == null ? '' : id); if (isPlausibleProfileId(s)) set.add(s); }
    return set.size === 1 ? [...set][0] : '';
  }
  // Which profile is the open overlay showing? Asked of the overlay's own React
  // fiber from up to three anchors — its largest photo, its heading, its
  // composer — which must agree. '' when no overlay is open, when no anchor
  // answers, or when they disagree; never a guess from elsewhere on the page.
  // The URL cannot say (?profile=true carries no id), the conversation Grindr
  // fetched can name whoever just messaged you, and the last /profiles/{id}
  // fetch is the pager's PREFETCH of the next profile — all three were tried
  // and all three pointed at someone else while a profile sat open.
  function openProfileViewId() {
    const root = findProfileOverlayRoot();
    if (!root) return '';
    // Anchors must belong to the pane itself. The pager's Previous/Next controls
    // carry 40px thumbnails of the NEIGHBOURING profiles, and after a hide leaves
    // the overlay pointing at a removed card the pane is blank — those thumbnails
    // were the only photos left, and a lone one "agreed" with itself, naming the
    // previous (or next) profile (caught live, v0.65.0 proof run). So: nothing
    // inside the pager controls, a photo has to be pane-sized, and at least two
    // anchors must answer before anything is believed.
    const inPager = (el) => !!(el && el.closest && el.closest(PROFILE_OVERLAY_MARKER));
    const usable = (el) => isVisibleEl(el) && !inPager(el) && !isOwnGreetUi(el);
    const anchors = [];
    try {
      const photos = Array.from(root.querySelectorAll(PROFILE_PHOTO_SELECTOR)).filter(usable)
        .filter((img) => img.getBoundingClientRect().width >= VIEW_PHOTO_MIN_PX)
        .sort((a, b) => b.getBoundingClientRect().width - a.getBoundingClientRect().width);
      if (photos[0]) anchors.push(photos[0]);
      const heading = Array.from(root.querySelectorAll('h1, h2, h3, h4, h5, h6')).find(usable);
      if (heading) anchors.push(heading);
      const composer = Array.from(root.querySelectorAll('input, textarea')).find(usable);
      if (composer) anchors.push(composer);
    } catch (_e) {}
    const me = String(albumState.myProfileId || '');
    const ids = anchors.map(profileIdFromViewFiber).filter((id) => id && id !== me);
    if (ids.length < VIEW_MIN_ANCHORS) return '';
    return pickAgreedId(ids);
  }

  // The element hosting an open full-screen profile, or null. The overlay is
  // recognised first by its pager arrows (findProfileOverlayRoot); the geometry
  ...
  function findOpenProfileView() {
    ...
    // The <=6-photo cap tells a profile apart from the cascade grid, which also
    // fills the viewport. v0.29–v0.64 lifted the cap when ?profile=true and then
    // fell back to "the largest visible container holding a photo". Both were
    // wrong in the same way: the first VISIBLE photo in DOM order is a cascade
    // tile scrolled behind the modal (top ≈ -828px is still "visible"), the walk
    // reached #cascade (3361px wide, 29 photos), and the caller then resolved the
    // FIRST GRID TILE as "the open profile". The overlay itself is found above,
    // by its pager arrows; when that misses there is no profile view to return.
  ...
  // Which profile do the action keys act on? By context, from the one source
  // that names the profile ON SCREEN in that context — and nothing else:
  //   overlay open — the route when it names the id (/profile/<id>, ?profileId=),
  //                  else the overlay's own React fiber (openProfileViewId).
  //   bare /chat    — the conversation Grindr itself fetched for that page.
  //   grid          — the tile under the pointer, else the keyboard cursor.
  // Sources that used to be tried here and were REMOVED in v0.65.0, each caught
  // naming someone else while a profile sat open (live session + 3 recordings):
  //   view     — resolveProfileIdFromClick(findOpenProfileView()): the view finder
  //              returned #cascade and the resolver read the FIRST GRID TILE.
  //   openPeer — the observed open conversation, which an inbox refresh sets to
  //              whoever just messaged you; kept for the /chat page only.
  //   recent   — lastViewedProfileId, the last /profiles/{id} Grindr fetched: the
  //              pager PREFETCHES the next profile, so it runs one step ahead.
  //   hover    — off the grid, a pointer resting on a tile behind the modal.
  // lastViewedProfileId is still tracked for the diagnostics report.
  let lastViewedProfileId = '';
  // Decide which profile the action keys act on. '' when the context cannot
  // name one — a missed keypress is recoverable, the wrong person is not.
  function resolveTargetProfileId() {
    ...
    const routeId = () => openProfileIdFromUrl();
    const viewId = () => openProfileViewId();
    const chatPeer = () => (isOnChatPage() ? chatPeerIdFromPath() : '');
    const cursor = () => (isPlausibleProfileId(hotkeyCursorId) ? hotkeyCursorId : '');
    const order = isProfileViewOpen() ? [routeId, viewId]
      : isOnChatPage() ? [chatPeer]
      : [hover, cursor];
  ...
  window.__grindrBlock_why = function () {
    ...
    return {
      ...
      overlayRootFound: !!findProfileOverlayRoot(),  // the MuiModal with the pager arrows (v0.65.0)
      openProfileViewId: openProfileViewId() || null, // who the overlay shows, per its own fiber
  ...
  // autoOpenChat: reveal the chat card as soon as a profile opens, instead of
  // making you click Chat every time. Off for a profile you have blocked, where
  // Grindr removes the button entirely — findProfileChatButton returns null there
  // and we simply do nothing rather than hunting the page for a substitute.
  const SETTINGS_DEFAULTS = {
    ...
    // Reveal the chat card as soon as a profile opens, rather than making you
    // click Chat every time.
    //
    // v0.63.0 shipped this key in SETTINGS_OPTIONS but NOT here, so
    // settings.autoOpenChat was undefined, autoOpenChatTick returned on its first
    // line, and the feature could never run. Nothing caught it: the options list
    // made it appear in the HUD, and no test asserts that every option has a
    // default. The test below now does.
    autoOpenChat: true,
  };
  ...
  const HUD_MINI_ID = 'grindr-block-hud-mini';   // the minimized badge, docked in the left rail
  ...
  let hudEl = null;
  let hudTimer = 0;
  let hudMiniMode = null;   // while minimized: 'rail' (docked) | 'corner' (no rail found) | null (open)
  ...
  // The default corner, before any drag is applied.
  function applyDefaultHudCorner() {
    if (!hudEl) return;
    hudEl.style.left = 'auto'; hudEl.style.top = 'auto'; hudEl.style.right = '12px';
    hudEl.style.bottom = IS_TOUCH ? 'calc(env(safe-area-inset-bottom,0px) + 72px)' : '12px';
  }

  // ── Minimized: a badge docked in Grindr's left rail ──────────────────────
  // Closing the panel used to leave a 29×28px ⌨ badge wherever the panel had
  // last been dragged — on a 3440px-wide screen it sat at x=3059 and could not
  // be found again. The rail is the one place that is always on screen and
  // always in the same spot, so the badge lives there while the panel is closed.
  //
  // Grindr's left rail: the fixed, full-height, ~64px column holding the logo,
  // inbox, favourites, your avatar and — at the bottom — the Terms of Service and
  // Privacy Policy links. Its classes are styled-components hashes, so it is
  // found from the one stable thing in it, the terms link, walked up to the first
  // position:fixed ancestor. Observed 2026-08-31: 64×1237px at left:0,
  // display:flex, column; four 64px items then a 222px legal block.
  function findLeftRail() {
    try {
      const link = document.querySelector('a[href*="grindr.com/terms"], a[href*="terms-of-service"]');
      let node = link ? link.parentElement : null;
      for (let i = 0; node && node !== document.body && i < 8; i += 1, node = node.parentElement) {
        const cs = getComputedStyle(node);
        if (cs.position !== 'fixed') continue;
        const r = node.getBoundingClientRect();
        if (r.left <= 4 && r.width <= 120 && r.height >= (window.innerHeight || 0) * 0.5) return node;
        return null;
      }
    } catch (_e) {}
    return null;
  }
  // The minimized badge: ⌨ normally, ● REC while recording. One click reopens.
  function makeHudBadge(inRail) {
    const badge = document.createElement('button');
    badge.type = 'button';
    if (inRail) badge.id = HUD_MINI_ID;
    badge.textContent = diagRecording ? '● REC' : '⌨';
    badge.title = 'GrindrBlock — click or press ' + HUD_TOGGLE_KEY + ' to open the panel';
    const color = 'color:' + (diagRecording ? '#ff6b6b' : '#e8e8e8');
    badge.style.cssText = inRail
      ? 'all:unset;box-sizing:border-box;display:flex;align-items:center;justify-content:center;width:100%;height:48px;cursor:pointer;font:13px ui-monospace,SFMono-Regular,Menlo,monospace;opacity:.9;' + color
      : 'all:unset;cursor:pointer;padding:6px 10px;display:block;font:12px ui-monospace,monospace;' + color;
    badge.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); hudOpen = true; persistHud(); renderHud(); });
    return badge;
  }
  // Mount (or re-mount after a React re-render) the minimized badge in the rail,
  // just above the legal block. True when it is in place.
  function ensureHudMini() {
    const rail = findLeftRail();
    if (!rail) { removeHudMini(); return false; }
    let mini = document.getElementById(HUD_MINI_ID);
    if (mini && mini.parentElement === rail) {
      const want = diagRecording ? '● REC' : '⌨';
      if (mini.textContent !== want) { mini.textContent = want; mini.style.color = diagRecording ? '#ff6b6b' : '#e8e8e8'; }
      return true;
    }
    if (mini) { try { mini.remove(); } catch (_e) {} }
    mini = makeHudBadge(true);
    const legal = Array.from(rail.children).find((c) => c.querySelector && c.querySelector('a[href*="terms"]'));
    if (legal) rail.insertBefore(mini, legal); else rail.appendChild(mini);
    return true;
  }
  // Take the badge out of the rail (the panel is open, or the rail is gone).
  function removeHudMini() {
    const mini = document.getElementById(HUD_MINI_ID);
    if (mini) { try { mini.remove(); } catch (_e) {} }
  }
  ...
  function renderHud() {
    ...
    hudEl.textContent = '';
    if (!hudOpen) {
      // Minimized. Dock the badge into the left rail (see ensureHudMini) and hide
      // the floating panel. Without a rail — the login page, an unfamiliar build —
      // the badge stays in this element, pinned to the bottom-LEFT corner rather
      // than wherever the panel was last dragged.
      if (ensureHudMini()) {
        hudMiniMode = 'rail';
      ...
      hudMiniMode = 'corner';
      hudEl.style.display = '';
      hudEl.style.left = '12px'; hudEl.style.top = 'auto'; hudEl.style.right = 'auto';
      hudEl.style.bottom = IS_TOUCH ? 'calc(env(safe-area-inset-bottom,0px) + 72px)' : '12px';
      hudEl.appendChild(makeHudBadge(false));
      return;
    }
    if (hudMiniMode) {
      // Reopening: put the panel back where the user left it (clamped into the
      // viewport), or in its default corner when it was never dragged.
      hudMiniMode = null;
      removeHudMini();
      hudEl.style.display = '';
      applyDefaultHudCorner();
      restoreHudPosition();
    }
  ...
  function installHud() {
    ...
    // A resize can strand a dragged panel off-screen; clamp it back in.
    window.addEventListener('resize', () => {
      if (!hudOpen || !hudEl) return;
      try { const r = hudEl.getBoundingClientRect(); placeHud(r.left, r.top); } catch (_e) {}
    });
    hudTimer = setInterval(() => {
      if (!hudEl) return;
      if (!hudOpen) {
        // Minimized: keep the rail badge in place across Grindr's re-renders (the
        // rail is React-owned and can drop foreign children), move into the rail
        // when one appears (after login), and keep its REC label current.
        const railNow = !!findLeftRail();
        const inRail = hudMiniMode === 'rail' && !!document.getElementById(HUD_MINI_ID);
        if (railNow !== inRail) { renderHud(); return; }
        if (inRail) ensureHudMini();
        return;
      }
  ...
  try {
    if (typeof module === 'object' && module && module.exports) {
      module.exports = {
        isPlausibleProfileId, conversationIdFor, isUsableHash, pickAgreedId,

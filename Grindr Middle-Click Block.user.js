...
(function () {
  ...
  const HOTKEY_GREET_KEY = ['Insert', 'F8'];
  ...
  function buildHar() {
    return {
      log: {
        version: '1.2',
        creator: { name: 'GrindrBlock userscript', version: '0.36.0' },
  ...
  function enforceAllBlocked() {
    ...
    for (const img of imgs) {
      ...
      if (!m) continue;
      let pid = photoHashToProfileId.get(m[1]);
      if (!pid) {
        // The hash index only covers profiles seen in a payload. A profile
        // resolved some other way (React fiber, attribute scan) has no hash
        // entry, so the sweep could never find its tile — the capture shows
        // exactly that: "confirm-DOM …: hashCards=0". Fall back to the same
        // resolver a real click uses, and remember the hash so the next sweep is
        // a cheap lookup again.
        const card0 = img.closest(CASCADE_CARD_SELECTOR);
        if (!card0) continue;
        let resolved = '';
        try { resolved = String((resolveProfileIdFromClick({ target: card0 }) || {}).profileId || ''); } catch (_e) {}
        if (!isPlausibleProfileId(resolved)) continue;
        if (!blockedSet.has(resolved) && !autoTextHiddenIds.has(resolved) && !hiddenProfileIds.has(resolved)) continue;
        cappedHashSet(m[1], resolved);
        pid = resolved;
      }
  ...
  function renderHud() {
    ...
    ]) keys.appendChild(hudRow(keyLabel(k), label));
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.36.0' });
  ...
  function saveDiagReport() {
    ...
    if (diagNet.length) {
      lines.push('');
      lines.push('network (newest last):');
      for (const r of diagNet.slice(-60)) {
        lines.push(`  ${String(r.status || '---').padStart(3)} ${r.method.padEnd(6)} ${String(r.ms).padStart(5)}ms  ${r.url.replace('https://web.grindr.com', '')}`
    ...
    try {
      ...
      // Second file: the network capture, as a real HAR.
      if (diagNet.length) {
        const hb = new Blob([JSON.stringify(buildHar(), null, 2)], { type: 'application/json' });
        const hu = URL.createObjectURL(hb);
        const ha = document.createElement('a');
        ha.id = 'grindr-block-diag-har';
        ha.href = hu;
        ha.download = `grindrblock-diag-${new Date().toISOString().replace(/[:.]/g, '-')}.har`;
        document.body.appendChild(ha);
        ha.click();
        setTimeout(() => { try { ha.remove(); URL.revokeObjectURL(hu); } catch (_e) {} }, 1000);
      }
      showToast(`Saved ${diagCount()} entries, ${diagNet.length} requests, ${diagClicks} clicks`, 'ok');
    } catch (e) {

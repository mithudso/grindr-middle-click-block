...
(function () {
  ...
  function buildHar() {
    return {
      log: {
        version: '1.2',
        creator: { name: 'GrindrBlock userscript', version: '0.44.0' },
  ...
  function hotkeyBlockTarget() {
    ...
    // like it worked.
    if (settings.hideCardOnBlock) {
      try {
        for (const c of findCardsForProfile(id)) {
          c.style.transition = 'opacity 0.2s';
          c.style.opacity = '0';
          c.style.display = 'none';
        }
        if (card && card.style) { card.style.opacity = '0'; card.style.display = 'none'; }
      } catch (e) { logTrace(`${LOG} immediate card hide failed:`, e); }
    }
  ...
  function renderHud() {
    ...
    tabBtn.type = 'button';
    tabBtn.textContent = hudTab === 'settings' ? '‹ back' : 'settings';
    ...
    tabBtn.addEventListener('click', () => { hudTab = hudTab === 'settings' ? 'main' : 'settings'; renderHud(); });
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.44.0' });

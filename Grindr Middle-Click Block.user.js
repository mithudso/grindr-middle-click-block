...
(function () {
  ...
  function buildHar() {
    return {
      log: {
        version: '1.2',
        creator: { name: 'GrindrBlock userscript', version: '0.42.0' },
  ...
  function findChatComposer() {
    ...
    for (const el of candidates) {
      ...
      // Nor the floating chat DRAWER's composer, when a profile overlay is open.
      // Live inspection found BOTH on screen at once — the profile's own
      // "Say something..." (input, 1152px) and the drawer's (textarea, 296px) —
      // and the scorer preferred the drawer purely because TEXTAREA earns +2,
      // which would have typed the greeting into whatever conversation the
      // drawer had open. That is a message to the wrong person.
      if (isProfileOverlayOpenFromUrl() && isInsideChatDrawer(el)) {
        logTrace(`${LOG} findChatComposer: skipping the chat drawer's composer while a profile is open`);
        continue;
      }
      let score = 0;
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.42.0' });

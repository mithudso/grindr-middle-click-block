...
(function () {
  ...
  function buildHar() {
    return {
      log: {
        version: '1.2',
        creator: { name: 'GrindrBlock userscript', version: '0.38.0' },
  ...
  function cardForImage(img) {
    ...
    for (let i = 0; node && i < 6; i += 1, node = node.parentElement) {
      ...
      if (r.width >= HOTKEY_MIN_TILE_PX && r.height >= HOTKEY_MIN_TILE_PX) return node;
    }
    // Nothing tile-sized above it — hide the image's immediate wrapper rather
    // than nothing at all.
  ...
  function enforceAllBlocked() {
    ...
    diagEvent('sweep', { imgs: imgs.length, matched: swept, hidden: collapsed, noCard, blocked: blockedSet.size, hidden_list: hiddenProfileIds.size });
  }
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.38.0' });

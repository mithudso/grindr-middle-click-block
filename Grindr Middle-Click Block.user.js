...
(function () {
  ...
  function diagPush(level, args) {
    ...
    diagBuffer.push({ t: Date.now(), level, msg: msg.slice(0, 2000) });
    if (diagBuffer.length > DIAG_MAX_ENTRIES) diagBuffer.shift();
  }
  ...
  function diagEvent(kind, detail) {
    if (!diagRecording) return;
    diagBuffer.push({ t: Date.now(), level: 'event', msg: `${kind} ${(() => { try { return JSON.stringify(detail); } catch (_e) { return String(detail); } })()}` });
    if (diagBuffer.length > DIAG_MAX_ENTRIES) diagBuffer.shift();
  }
  ...
  function enforceAllBlocked() {
    ...
    const pending = pendingBlockIds();
    const pendingSet = new Set(pending);
    if (!pendingSet.size && !autoTextHiddenIds.size && !hiddenProfileIds.size) return;
    ...
    for (const img of imgs) {
      ...
      if (!pid) continue;
      const isBlocked = pendingSet.has(pid);
  ...
  window.__grindrBlock_state = function () {
    ...
    return {
      ...
      blocksConfirmed: blockConfirmedIds.size,      // propagated server-side; never scanned
  ...
  function renderHud() {
    ...
    for (const [k, label] of [
      [HOTKEY_GREET_KEY, 'greet'],
      [HOTKEY_ALBUM_KEY, 'unlock album'],
      [HOTKEY_BLOCK_KEY, 'block'],
      [HOTKEY_HIDE_KEY, 'hide'],
      [HOTKEY_PREV_KEY, 'previous profile'],
      [HOTKEY_NEXT_KEY, 'next profile'],
    ]) keys.appendChild(hudRow(k, label));
    ...
    diag.appendChild(mkBtn('reconcile', () => {
      reconcileBlockTiers(true).then(() => renderHud()).catch(() => {});
    }));
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.33.0' });
  ...
  function stopDiagRecording() {
    if (!diagRecording) return;
    diagEvent('recording-stopped', { entries: diagBuffer.length });
    diagRecording = false;
    logInfo(`${LOG} diagnostic recording stopped (${diagBuffer.length} entries). Press save to download.`);
  }
  ...
  function saveDiagReport() {
    ...
    ]) lines.push(`  ${k.padEnd(10)} ${label}`);
    lines.push('');
    try { lines.push(`why(): ${JSON.stringify(window.__grindrBlock_why(), null, 2)}`); } catch (_e) {}
    lines.push('');
    lines.push(`timeline (${diagBuffer.length} entries${diagStartedAt ? `, ${Math.round((Date.now() - diagStartedAt) / 1000)}s` : ''}):`);
    const t0 = diagBuffer.length ? diagBuffer[0].t : Date.now();
    for (const e of diagBuffer) {
    ...
    try {
      ...
      showToast(`Saved ${diagBuffer.length} entries`, 'ok');
    } catch (e) {
  ...
  function installHud() {
    ...
    hudTimer = setInterval(() => { if (hudOpen && hudEl) renderHud(); }, 1000);
  ...
  window.__grindrBlock_hud = function (show) {
    ...
    return { open: hudOpen, recording: diagRecording, entries: diagBuffer.length };
  };

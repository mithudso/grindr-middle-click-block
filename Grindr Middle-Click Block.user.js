...
(function () {
  ...
  function buildHar() {
    return {
      log: {
        version: '1.2',
        creator: { name: 'GrindrBlock userscript', version: '0.48.0' },
  ...
  function hidesNeedingUpgrade() {
    const out = [];
    for (const id of blockedProfileIds) if (!serverBlockedIds.has(id)) out.push(id);
    return out;
  }
  ...
  function installAutoDrain() {
    ...
    autoDrain = !!(o && o.on);
    drainTimer = setInterval(() => { drainTick().catch((e) => logTrace(`${LOG} drainTick error:`, e)); }, DRAIN_TICK_MS);
  ...
  function upgradeHidesToBlocks(limit) {
    ...
    for (const id of todo) enqueueAction(id, 'block');
  ...
  async function runBlockJob(job, auth, jobKey) {
    const { profileId, action } = job;
        ...
        if (stuck) {
          ...
          if (BLOCK_MODE === 'block') { try { serverBlockedIds.add(String(profileId)); refreshHud(); } catch (_e) {} }
  ...
  function startDiagRecording() {
    ...
    diagEvent('recording-started', { url: location.href, version: '0.48.0' });
